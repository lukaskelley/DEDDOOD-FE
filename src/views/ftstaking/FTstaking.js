...
const FTstaking = () => {
  ...
  const updateEthers = async () => {
    ...
    let ftStakingContract = new ethers.Contract(
      config.FTStakingAddress,
      ftStakingABI,
      tempSigner
    );
  ...
  const claimState = async () => {
    ...
    await ftStakingContract
      .stakingInfos(defaultAccount.toString())
      .then((stakeInfo) => {
  ...
  const staking = async (amount, day) => {
    ...
    if (stakeState) {
      ...
      const amountValue = ethers.BigNumber.from(amount).mul(
  ...
  return (
    <div className="ftStakingContainer p-1 m-3">
      <CRow className="p-1 m-3 btnConnectConbtainer">
      ...
      <CRow className="align-items-start">
        <CCol className="infoContain-topbar">sDED STAKING</CCol>
      </CRow>
      {!stakeState ? (
        <>
          <CRow className="justify-content-around">
            <CCol lg={4} xs={23} sm={12} md={12} className="ftinfoContain">
              ...
              <input
                type="number"
                className="ftstakingInput"
                onChange={InputValue_30}
              ></input>
              <button
                className="ftstakeBtn"
