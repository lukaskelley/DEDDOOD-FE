...
const NFTCard = ({ tokenId, isStaked, balance, src, tier, level }) => {
  ...
  const updateEthers = async () => {
    ...
    let sDoodContract = new ethers.Contract(config.sDoodAddress, sdoodABI, tempSigner)
  ...
  const upgrade = async () => {
    if (Number(level) > 3) {
      ...
      if (amountValue > sdoodContract.balanceOf(balanceToken)) {
        ...
        await sdoodContract
          .approve(config.DEDNFTStakingAddress, amountValue, { gasLimit: 3000000 })
          .then((tx) => {
            tx.wait().then(() => {
              nftStakingContract.UpgradeLevel(tokenId, { gasLimit: 3000000 }).then((tx2) => {
                tx2.wait().then(() => {
                  Modal.success({
    ...
    if (Number(level) > 3) {
      ...
      if (amountValue > sdoodContract.balanceOf(balanceToken)) {
        ...
        await sdoodContract
          ...
          .then((tx) => {
            tx.wait().then(() => {
              nftStakingContract.UpgradeLevelMax(tokenId, { gasLimit: 3000000 }).then((tx2) => {
                tx2.wait().then(() => {
                  Modal.success({
                    content: 'Upgrade Successful',
                    onOk() {
