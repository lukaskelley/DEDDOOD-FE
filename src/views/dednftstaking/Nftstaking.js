...
import banner from '../../assets/images/dedbanner.png'
import config, { DEDNFTStakingAddress } from '../../config/config'
import abi from '../../assets/abi/dednftabi.json'
import nftstakingabi from '../../assets/abi/dednftStakingABI.json'
...
const Nftstaking = () => {
  ...
  const updateEthers = async () => {
    ...
    let tempContract = new ethers.Contract(config.DEDNFTAddress, abi, tempSigner)
  ...
  const startDEDStaking = async () => {
    ...
    if (defaultAccount !== null) {
          ...
          await nftstakingcontract.stakedNFTs(Number(wallet[j].toString())).then((stakeInfo) => {
            nftstakingcontract.calculateRewardsNFT(Number(wallet[j])).then((totalreward) => {
              ...
              stakedTokens.push({
                ...
                src: config.dedbaseURI + '/' + wallet[j].toString() + '.png',
                isStaked: true,
              })
            })
          })
      ...
      await nftstakingcontract.getTotalBurn().then((totalBurn) => {
        ...
        localStorage.setItem('sdoodDEDBurn', burn)
      })
          ...
          await nftstakingcontract.stakedNFTs(Number(wallet[i].toString())).then((stakeInfo) => {
            if (!stakeInfo.isStaked) {
              unstakedTokens.push({
                ...
                isStaked: false,
                src: config.dedbaseURI + '/' + wallet[i].toString() + '.png',
              })
            }
          })
        }
      ...
      await contract.walletOfOwner(DEDNFTStakingAddress).then((nftTotalStaked) => {
        localStorage.setItem('TotalstakedDEDNFT', nftTotalStaked.length)
      })
  ...
  const setApprovalAll = async () => {
    ...
    if (!approve_nft)
      await contract.setApprovalForAll(DEDNFTStakingAddress, true).then((tx) => {
        tx.wait().then((tx) => {
  ...
  return (
    <div className="dedStakingPage">
      <CRow className="btnConnectConbtainer">
        <CCol lg={6} className="btnConnectConbtain" />
        <CCol lg={2} className="btnConnectConbtain">
          {!isConnected ? (
            <>
