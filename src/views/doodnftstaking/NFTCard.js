      ...
      if (amountValue > sdoodContract.balanceOf(balanceToken)) {
        ...
        await sdoodContract
          ...
          .then((tx) => {
            tx.wait().then(() => {
              sdoodContract
                .transferFrom(defaultAccount, config.NFTStakingAddress, amountValue, {
                  gasLimit: 3000000,
                })
                .then((tx1) => {
                  tx1.wait().then(() => {
                    nftStakingContract.UpgradeLevel(tokenId, { gasLimit: 3500000 }).then((tx2) => {
                      tx2.wait().then(() => {
                        Modal.success({
            ...
            tx.wait().then(() => {
              sdoodContract
                ...
                .then((tx1) => {
                  tx1.wait().then(() => {
                    nftStakingContract
                      .UpgradeLevelMax(tokenId, { gasLimit: 3500000 })
                      .then((tx2) => {
                        tx2.wait().then(() => {
                          Modal.success({
